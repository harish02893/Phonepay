...
if SELECT == "Basic insights":
    ...
    options = ["--select--",
               ...
               #1cursor.execute("SELECT DISTINCT States, Transaction_Year, SUM(Transaction_Amount) AS Total_Transaction_Amount FROM top_trans GROUP BY States, Transaction_Year ORDER BY Total_Transaction_Amount DESC LIMIT 10")

               
    select = st.selectbox("Select the option",options)
    
    if select == "Top 10 states based on year and amount of transaction":
        cursor.execute("DESCRIBE top_trans")
        column_info = cursor.fetchall()
        print(column_info)  # Check the console for column information
        
        cursor.execute("SELECT DISTINCT State, Year, SUM(Transaction_amount) AS Transaction_amount FROM top_trans GROUP BY State,Year ORDER BY Transaction_amount DESC LIMIT 10")

        
        data = cursor.fetchall()
        df = pd.DataFrame(data, columns=['State', 'Year', 'Transaction_amount'])
        ...
        with col2:
            ...
            st.bar_chart(df,x="Total_Transaction",y="State")
        ...
        with col1:
            st.write(df)
        with col2:
            st.title("List 10 Count based on Districts and states")
            st.bar_chart(data=df,y="State",x="Count")
        ...
        with col1:
            st.write(df)
        with col2:
            st.title("Top 10 Registered_user based on state and District")
            st.bar_chart(data=df,y="State",x="Registered_user")
...
if SELECT == "Home":
    ...
    col1.image(Image.open("D:/capstone phonepay project/images.png"),width = 200)
    with col1,col2:
    ...
    with col1:
        st.video("D:/capstone phonepay project/upi.mp4")
        
    st.subheader(':blue[Registered Users Hotspots - States]')
    #col2.image(Image.open("D:/capstone phonepay project/phn.png"),width = 50)


    Data_Aggregated_Transaction_df= pd.read_csv(r'D:/capstone phonepay project/Data_Aggregated_Transaction_Table.csv')
    Data_Aggregated_User_Summary_df= pd.read_csv(r'D:/capstone phonepay project/Data_Aggregated_User_Summary_Table.csv')
    Data_Aggregated_User_df= pd.read_csv(r'D:/capstone phonepay project/Data_Aggregated_User_Table.csv')
    Scatter_Geo_Dataset =  pd.read_csv(r'D:/capstone phonepay project/Data_Map_Districts_Longitude_Latitude.csv')
    Coropleth_Dataset =  pd.read_csv(r'D:/capstone phonepay project/Data_Map_IndiaStates_TU.csv')
    Data_Map_Transaction_df = pd.read_csv(r'D:/capstone phonepay project/Data_Map_Transaction_Table.csv')
    Data_Map_User_Table= pd.read_csv(r'D:/capstone phonepay project/Data_Map_User_Table.csv')
    Indian_States= pd.read_csv(r'D:/capstone phonepay project/Longitude_Latitude_State_Table.csv')
...
if SELECT == "About":
    col1,col2 = st.columns(2)
    with col1:
        st.video("D:/capstone phonepay project/vid.mp4")
    with col2:
        st.image(Image.open("D:/capstone phonepay project/PhonePe_Logo.jpg"),width = 500)
        st.write("---")
    ...
    with col1:
        ...
        st.image(Image.open("D:/capstone phonepay project/top.jpeg"),width = 400)
        with open("D:/capstone phonepay project/annual report.pdf","rb") as f:
            data = f.read()
        st.download_button("DOWNLOAD REPORT",data,file_name="annual report.pdf")
    with col2:
        st.image(Image.open("D:/capstone phonepay project/report.jpeg"),width = 200)
...
if SELECT == "Contact":
    st.subheader("Kesavan sekar")
    name = "Kesavan sekar"
    mail = (f'{"Mail :"}  {"harishu02893@gmail.com"}')
    description = "An Aspiring DATA-SCIENTIST..!"
    social_media = {"GITHUB": "https://github.com/harish02893"}
    
    col1, col2, col3 = st.columns(3)
    col1.image(Image.open("D:/capstone phonepay project/136705305.png"), width=180)
    with col2:
